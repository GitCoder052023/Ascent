...
import { setCachedWifi } from "../src/lib/recordingContext.ts";
...
const device = { platform: "android", deviceModel: "RMX3853", osVersion: "36" };

const wifiData = {
  ssid: "The Muscle Asylum Gym",
  bssid: "A8:88:1F:73:30:3A",
  signalStrength: -44,
...
assert.equal(accel.ssid, "The Muscle Asylum Gym");
assert.equal(accel.bssid, "A8:88:1F:73:30:3A");
assert.equal(accel.signalStrength, -44);
assert.equal(accel.signalStrengthUnit, "dBm");
assert.equal(accel.frequency, 2452);
assert.equal(accel.connectionType, "wifi");
...
// Update Wi-Fi signal strength (e.g. RSSI change to -51)
setCachedWifi({ signalStrength: -51 });
...
assert.equal(gyro.ssid, "The Muscle Asylum Gym");
assert.equal(gyro.signalStrength, -51);
...
assert.equal(later.ssid, "The Muscle Asylum Gym");
assert.equal(later.signalStrength, -51);
...
assert.equal(wifi.signalStrength, -47);
...
assert.equal(csv.includes("The Muscle Asylum Gym"), true);
assert.equal(csv.includes("A8:88:1F:73:30:3A"), true);
assert.equal(csv.includes("-44"), true);
assert.equal(csv.includes("2452"), true);
assert.equal(csv.includes("dBm"), true);
